...
@.planning/phases/05-config-loader-shared-hedging-models/05-02-PLAN.md
...
NOTE: Import `scan_chain_quiet` from rolling_tracker inside the `validate_budget` method (lazy import) to avoid circular issues if rolling_tracker is not yet built. Use:
```python
...
- scan_chain_quiet integration for live premium estimation
  </done>
</task>
...
5. Budget tracker auto-resets on new month
6. No argparse or CLI code in this Layer 2 file
</success_criteria>

<output>
After completion, create `.planning/phases/07-rolling-tracker-hedge-sizer/07-02-SUMMARY.md`
</output>
