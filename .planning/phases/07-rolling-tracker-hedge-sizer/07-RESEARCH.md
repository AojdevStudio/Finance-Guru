# Phase 7: Rolling Tracker & Hedge Sizer - Research
...
The established 3-layer architecture (Models -> Calculator -> CLI) applies to both tools. Phase 5 creates the required Pydantic models (`HedgePosition`, `RollSuggestion`, `HedgeSizeRequest`) and config infrastructure (`load_hedge_config()`, `HedgeConfig`). Phase 7 consumes these directly. No new dependencies are required.
...
|---------|---------|---------|--------------|
| pydantic | 2.12.0 | Model validation (HedgePosition, RollSuggestion, HedgeSizeRequest, HedgeConfig) | Already installed; Phase 5 creates the models |
...
src/
  models/
    hedging_inputs.py        # [Phase 5] HedgePosition, RollSuggestion, HedgeSizeRequest
  config/
    config_loader.py         # [Phase 5] HedgeConfig, load_hedge_config()
  analysis/
    rolling_tracker.py       # [Phase 7] RollingTracker calculator class (Layer 2)
    rolling_tracker_cli.py   # [Phase 7] CLI with subcommands (Layer 3)
    hedge_sizer.py           # [Phase 7] HedgeSizer calculator class (Layer 2)
    hedge_sizer_cli.py       # [Phase 7] CLI interface (Layer 3)
    options_chain_cli.py     # [Existing] scan_chain() function to import
    options.py               # [Existing] price_option() for BS pricing

fin-guru/
  data/
    hedging-strategies.md          # [Phase 7] Knowledge base - hedging strategies
    options-insurance-framework.md # [Phase 7] Knowledge base - options as insurance
  agents/
    strategy-advisor.md      # [Phase 7] Add knowledge file references
    teaching-specialist.md   # [Phase 7] Add knowledge file references
    quant-analyst.md         # [Phase 7] Add knowledge file references

fin-guru-private/
  hedging/
    positions.yaml           # [Phase 5 scaffolds] Active positions (Phase 7 reads/writes)
    roll-history.yaml        # [Phase 5 scaffolds] Roll records (Phase 7 appends)
    budget-tracker.yaml      # [Phase 5 scaffolds] Budget tracking (Phase 7 updates)

tests/
  python/
    test_rolling_tracker.py  # [Phase 7] Known-answer tests
    test_hedge_sizer.py      # [Phase 7] Known-answer tests
```
...
```python
# Source: Codebase pattern from src/config.py + Phase 5 template design
import yaml
...
| Config loading | Direct YAML parsing in CLI | `load_hedge_config()` from config_loader.py | DRY bridge with override chain, Phase 5 tested |
...
**Key insight:** Phase 7 is primarily an *integration* phase. The heavy lifting (options chain scanning, BS pricing, config loading, model validation) is already built. Phase 7 wires these together into two user-facing CLIs.
...
**How to avoid:** HedgeConfig already validates weights sum near 1.0 (Phase 5 model_validator). When CLI passes `--underlyings` as a comma-separated list, use the config weights for those underlyings only. If the requested underlyings are not all in the weight config, fall back to equal weighting.
...
```python
# Source: Phase 5 config_loader.py pattern (from 05-02-PLAN.md)
from pathlib import Path
...
4. **Dividend yield for BS pricing of puts on ETFs**
   ...
   - Recommendation: Use 0.0 for v1. The dividend yield effect on put pricing is small for ETFs with <2% yield. Document this as a known simplification. Phase 8 or a future iteration can add dividend yield lookup if needed.
...
- Phase 5 plans: `05-01-PLAN.md` (HedgePosition, RollSuggestion, HedgeSizeRequest model specs), `05-02-PLAN.md` (config_loader.py, YAML templates)
- Phase 6 plans: `06-01-PLAN.md` (TotalReturnCalculator pattern), `06-02-PLAN.md` (total_return_cli.py argparse pattern)
...
- YAML state management: HIGH -- Pattern verified from existing config.py and Phase 5 template designs
