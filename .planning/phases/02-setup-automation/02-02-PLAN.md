...
must_haves:
  truths:
    ...
    - "Running ./setup.sh a second time skips completed steps, shows 'Resuming from step N/M', and only prompts for missing config files (file-level idempotency — field-level YAML merging is Phase 3 onboarding wizard scope)"
    - "Running ./setup.sh a second time with existing user-profile.yaml prompts 'Overwrite? [y/N]' and defaults to keeping the existing file (file-level skip/prompt, not field-level merge)"
    - "Directory structure is validated on EVERY run (first run and re-run alike) — missing subdirectories are always detected and recreated"
...
- If .setup-progress exists, counts completed steps vs total (5 trackable milestones: deps_checked, dirs_created, dirs_verified, config_scaffolded, python_deps_installed — note: dirs_verified is always set alongside dirs_created on first run, or on its own during re-run verification)
...
**SCOPE BOUNDARY (ONBD-06):** For Phase 2, idempotency is FILE-LEVEL: skip existing files or prompt for full overwrite. The scaffold_file() function does NOT parse existing YAML/env files to selectively update missing fields within them. Field-level updates (e.g., parsing user-profile.yaml to add only missing keys while preserving user-entered values) belong to the Phase 3 onboarding wizard. CONTEXT.md confirms: "setup.sh prepares the environment, it does not collect financial profile data."
...
grep -q "dirs_verified" .setup-progress && echo "DIRS VERIFIED TRACKED"
...
3. **Partial re-run (no progress file, dirs exist):** Delete .setup-progress, but leave directories in place. Run `./setup.sh`. Verify:
   - Dependency check runs (no cached result)
   - Directory creation says "Already exists" for each dir (mkdir -p is idempotent)
   - verify_directory_structure() ALSO runs (validates expected structure even on first-run path)
   - Config scaffolding prompts for overwrite (or skips in non-interactive)
   - .setup-progress is rebuilt with all steps including dirs_verified
...
# Verify dirs_verified is tracked on both first-run and re-run paths
grep -q "dirs_verified" .setup-progress && echo "DIRS VERIFIED TRACKED"
...
- SC4: ./setup.sh second run detects existing configuration, validates directory structure on every path (first run + re-run), and prompts for file-level overwrite only (Task 1 main flow + Task 2 + Task 3)

Requirements covered:
- ONBD-05: setup.sh orchestrates full first-time setup (dependency checks, directory creation, config generation)
- ONBD-06: setup.sh is idempotent -- file-level: re-run skips existing files or prompts for overwrite; directory-level: validates expected structure on every run. Field-level YAML merging deferred to Phase 3 onboarding wizard (CONTEXT.md: "setup.sh prepares the environment, it does not collect financial profile data")
...
- Directory structure validated on EVERY run (first run calls verify after create; re-run calls verify directly)
- Existing config files prompt before overwrite, default to keep (file-level idempotency; field-level merging deferred to Phase 3)
