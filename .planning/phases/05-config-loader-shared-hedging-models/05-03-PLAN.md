...
Purpose: These tests are the acceptance gate for Phase 5. They verify every success criterion: model validation, config loading, graceful fallback, and CLI override behavior. Phase 6-8 executors need confidence that the foundation is solid.
...
@.planning/phases/05-config-loader-shared-hedging-models/05-01-SUMMARY.md
@.planning/phases/05-config-loader-shared-hedging-models/05-02-SUMMARY.md
...
```python
"""Tests for hedging Pydantic models (Phase 5, HEDG-02)."""
import pytest
...
```python
"""Tests for total return Pydantic models (Phase 5, HEDG-03)."""
import pytest
...
```python
"""Tests for config loader (Phase 5, CFG-01/02/03)."""
import pytest
import yaml
...
Keep tests focused on the override chain. Do NOT test network/yfinance (Phase 5 is pure models and config).
  </action>
  <verify>
```bash
...
<verification>
Run full Phase 5 acceptance gate:
...
<output>
After completion, create `.planning/phases/05-config-loader-shared-hedging-models/05-03-SUMMARY.md`
</output>
