# Phase 5: Config Loader & Shared Hedging Models - Research
...
Phase 5 establishes the shared foundation that all four hedging CLI tools (total return, rolling tracker, hedge sizer, SQQQ comparison) depend on. The research confirms this is entirely a "standard patterns" phase -- every building block uses libraries and conventions already present in the codebase.
...
src/
  ...
  analysis/
    (future Phase 6-8 CLIs will import from models/ and config/)
...
- Phase 7+ may add more config utilities (position persistence, budget tracking)
...
**When to use:** Every hedging CLI built in Phases 6-8
**Example:**
```python
...
**Why it happens:** Anticipating Phase 6-8 needs before those phases are planned
**How to avoid:** Start with the parameters listed in the requirements: budget, roll_window, underlying_weights. Phases 6-8 can extend HedgeConfig with additional fields (Pydantic models are easy to extend). The YAML `hedging:` section should only contain fields that HedgeConfig validates today
...
- **Phase 3 (user-profile.yaml):** The hedging section schema must be defined here (Phase 5), but the actual YAML file is generated by Phase 3 onboarding. Phase 5 must work WITHOUT the onboarding having run (CFG-03). The hedging section design in Phase 5 becomes the specification that Phase 3 will implement.
...
- **Phase 6 (Total Return):** Imports TotalReturnInput, DividendRecord, TickerReturn from total_return_inputs.py. Imports load_hedge_config from config_loader.py.
- **Phase 7 (Tracker & Sizer):** Imports HedgePosition, RollSuggestion, HedgeSizeRequest from hedging_inputs.py. Reads/writes to fin-guru-private/hedging/ YAML files.
- **Phase 8 (SQQQ Comparison):** Imports HedgeConfig for SQQQ parameters. Uses hedging_inputs.py models.

### No Dependency On
- options.py, options_chain_cli.py (those are Phase 7 integration points, not Phase 5)
- yfinance, market_data.py (Phase 5 is pure models and config, no market data fetching)
...
2. **HedgeConfig field set completeness**
   - What we know: Requirements list budget, roll_window, underlying_weights as explicit fields. Phase 6-8 may need additional config fields.
   - What's unclear: Exactly which fields Phase 8 (SQQQ comparison) will need in HedgeConfig.
   - Recommendation: Include the fields documented in the YAML template design above. Phase 8 can add fields to HedgeConfig when it is planned (Pydantic models are trivially extensible with new optional fields).
...
3. **HedgePosition relationship to OptionContractData**
   ...
   - Recommendation: Keep them separate. OptionContractData is a market data snapshot (bid/ask/greeks from live scan). HedgePosition is an owned position (entry price, quantity, P&L). Different lifecycle, different fields. Phase 7 bridges them when generating roll suggestions.
...
- `.planning/ROADMAP.md` -- Phase 5 requirements, dependency chain, success criteria
...
- Domain models: MEDIUM -- Model field sets are inferred from requirements and Phase 7/8 success criteria; may need refinement during those phases
