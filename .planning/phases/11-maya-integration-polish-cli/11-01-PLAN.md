...
@.planning/ROADMAP.md
@.planning/STATE.md
@.planning/phases/10-self-assessment-persistence-topics/10-02-PLAN.md

Phase 10 plan 02 provides the template structure this plan modifies (explorer.ts, template.html).
Phase 10 adds localStorage persistence and 4-state knowledge cycling -- this plan builds the export on top of that persisted state.
...
1. **Export Profile button** in the header action area (near the existing Export/Copy buttons):
   ...
   ```
   Place after the existing export-btn. Use a distinct ID (`export-profile-btn`) to differentiate from the Phase 10 `export-btn` (which exports raw localStorage progress). This button exports a structured Maya-compatible learner profile.
...
2. **`buildLearnerProfile()` function** that constructs the Maya-compatible JSON:
   ...
   function buildLearnerProfile(): object {
     ...
     for (const node of nodes) {
       const state = getNodeKnowledge(node.id); // from Phase 10 state management
     ...
     return {
       learner_profile: {
         ...
         learning_mode_preference: getCurrentLearningMode(), // from Phase 10
...
5. **Helper functions** (if not already present from Phase 10):
   ...
   - These should already exist from Phase 10's localStorage persistence and learning mode features. If they exist under different names, use the existing function names.
...
- Do NOT break any existing Phase 10 functionality (persistence, learning modes, clipboard, export progress)
- The `export-profile-btn` is SEPARATE from Phase 10's `export-btn` (which exports raw progress)
...
3. Build check:
```bash
cd /Users/ossieirondi/Documents/Irondi-Household/family-office && bun build src/explorer/template/explorer.ts --outdir /tmp/build-check-11-01 --format iife --target browser 2>&1
```
  </verify>
  <done>
...
- No existing Phase 10 features broken
  </done>
</task>
...
- All changes are additive -- zero breakage of Phase 10 features or existing Maya behavior
</success_criteria>

<output>
After completion, create `.planning/phases/11-maya-integration-polish-cli/11-01-SUMMARY.md`
</output>
