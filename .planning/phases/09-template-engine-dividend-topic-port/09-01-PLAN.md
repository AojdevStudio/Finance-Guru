...
must_haves:
  truths:
    - "Zod topic schema validates well-formed JSON and rejects malformed JSON with clear error messages"
    ...
    - "HTML template has injection points for CSS, topic data, and application JS"
    - "CSS styles produce the dark theme layout matching the prototype (header, sidebar, graph container, prompt panel)"
  artifacts:
...
<task type="auto">
  ...
  <verify>
    ...
    grep -c "__INJECT_CSS__" src/explorer/template/template.html    # Must be 1
    grep -c "__INJECT_TOPIC_DATA__" src/explorer/template/template.html  # Must be 1
    grep -c "__INJECT_APP_JS__" src/explorer/template/template.html      # Must be 1
    grep -c "__TOPIC_TITLE__" src/explorer/template/template.html        # Must be 2 (title tag + h1)

    # Verify cy container has explicit dimensions
    grep -c "#cy" src/explorer/template/styles.css  # Must be >= 1

    # Verify dist is gitignored
    git check-ignore src/explorer/dist/test.html && echo "dist gitignored: OK" || echo "dist gitignored: MISSING"
    ```
  </verify>
  <done>
...
```bash
# Schema compiles
bun build src/explorer/schemas/topic-schema.ts --outdir /tmp/verify-09-01 --target node 2>&1
...
<output>
After completion, create `.planning/phases/09-template-engine-dividend-topic-port/09-01-SUMMARY.md`
</output>
