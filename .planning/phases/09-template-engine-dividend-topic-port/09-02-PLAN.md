...
Purpose: This is the core application logic that transforms static topic data into an interactive knowledge graph. It is the most complex piece of Phase 9 and must implement all 14 features identified in the prototype analysis.
...
<task type="auto">
  ...
  <verify>
    ...
    wc -l src/explorer/template/explorer.ts  # Should be 200+ lines

    # Verify it imports cytoscape
    grep "import cytoscape" src/explorer/template/explorer.ts

    # Verify it references TOPIC_DATA
    grep "TOPIC_DATA" src/explorer/template/explorer.ts

    # Verify it compiles (as IIFE target, like the build will do)
    bun build src/explorer/template/explorer.ts --format iife --target browser --outdir /tmp/explorer-check 2>&1 | head -10
    ```
    The file must compile without errors as IIFE format targeting browser.
  </verify>
  <done>
...
# Compiles as IIFE
bun build src/explorer/template/explorer.ts --format iife --target browser --outdir /tmp/verify-09-02 2>&1
...
<output>
After completion, create `.planning/phases/09-template-engine-dividend-topic-port/09-02-SUMMARY.md`
</output>
