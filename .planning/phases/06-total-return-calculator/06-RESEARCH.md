# Phase 6: Total Return Calculator - Research
...
Phase 6 adds a total return calculator CLI that computes three distinct metrics per ticker -- price return, dividend return, and total return -- with optional DRIP (dividend reinvestment) modeling. The tool follows the established 3-layer architecture already used by 8 existing CLI tools in the codebase. Phase 5 creates the Pydantic models (`TotalReturnInput`, `DividendRecord`, `TickerReturn`) and `config_loader.py` that this phase consumes.
...
src/
  models/
    total_return_inputs.py     # [Phase 5 creates] TotalReturnInput, DividendRecord, TickerReturn
  analysis/
    total_return.py            # [Phase 6] TotalReturnCalculator class (Layer 2)
    total_return_cli.py        # [Phase 6] CLI interface (Layer 3)
  utils/
    config_loader.py           # [Phase 5 creates] HedgeConfig from user-profile.yaml
tests/
  python/
    test_total_return.py       # [Phase 6] Known-answer tests
```
...
Layer 1 (Models -- created by Phase 5):
```python
# src/models/total_return_inputs.py (Phase 5 responsibility)
...
**Version note:** Project pinned at `yfinance>=0.2.66` in pyproject.toml. Both 0.2.66 and 1.1.0 work for this use case. No need to force upgrade for Phase 6; the dividend API is the same across versions.

## Open Questions

Things that couldn't be fully resolved:

1. **Phase 5 Model Exact Schema**
   - What we know: Phase 5 creates `TotalReturnInput`, `DividendRecord`, `TickerReturn` in `src/models/total_return_inputs.py`
   - What's unclear: Exact field names and validators (Phase 5 has no plans yet)
   - Recommendation: Phase 6 planner should define the ideal model shape and communicate to Phase 5 if Phase 5 has not yet been planned. The models shown in this research are the recommended shapes.
...
- Phase 5 dependency: MEDIUM - Model shapes recommended but Phase 5 not yet planned; exact field names may differ
