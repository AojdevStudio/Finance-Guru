...
must_haves:
  truths:
    - "No tracked file in the working tree contains the literal account number Z05724592"
    - "No tracked file contains hardcoded personal names (Ossie, Irondi) except template variable references like {user_name}"
    - "No tracked file contains the Google Sheets spreadsheet ID 1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4"
    - "No tracked file contains employer names (Avanade, CBN) in PII context"
...
<task type="auto">
  ...
  <action>
    ...
    **Account numbers:**
    - `Z05724592` -> `{account_id}` in code/config files, `EXAMPLE_ACCOUNT_123` in docs/examples
    - `Balances_for_Account_Z05724592` -> `Balances_for_Account_{account_id}`
    - `History_for_Account_Z05724592` -> `History_for_Account_{account_id}`

    **Google Sheets spreadsheet ID:**
    - `1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4` -> `{spreadsheet_id}` in code, `YOUR_SPREADSHEET_ID` in docs

    **Personal names:**
    - `Ossie Irondi` -> `{user_name}` in code/templates, remove from docs or replace with generic "the user"
    - `Ossie` (standalone, in PII context) -> `{user_name}` in code, generic reference in docs
    - Be CAREFUL: Do NOT replace "Ossie" inside file paths that reference this as a username on disk (those are already in .gitignore or will be excluded). Only replace in content that would appear in the public repo.

    **Employer names:**
    - `Avanade` -> `{employer_name}` or remove from context
    - `CBN` in employer context (e.g., "CBN 401(k)", "CBN 401k") -> `{employer_name}` or remove. Do NOT replace standalone "CBN" that is not in employer context.

    **LLC names (if in working tree):**
    - `MaryFinds LLC` -> `{llc_name}` or remove
    - `KC Ventures Consulting Group LLC` -> `{llc_name}` or remove

    **Approach:**
    1. First, run `git ls-files | xargs grep -l "Z05724592"` to get the current list of affected files (research found 13).
    2. Also run `git ls-files | xargs grep -l "1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4"` for spreadsheet ID files.
    3. Also run `git ls-files | xargs grep -l "Ossie\|Irondi"` for personal name files.
    4. Also run `git ls-files | xargs grep -l "Avanade\|CBN"` for employer name files.
    ...
    **Do NOT modify:**
    ...
    - Files that only mention "Ossie" in an OS-level path context (like `/Users/ossieirondi/`)
  </action>
  <verify>
    Run ALL of these and verify zero matches:
    ```bash
    git ls-files | xargs grep -l "Z05724592" 2>/dev/null | wc -l  # Must be 0
    git ls-files | xargs grep -l "1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4" 2>/dev/null | wc -l  # Must be 0
    git ls-files | xargs grep -rl "\bAvanade\b" 2>/dev/null | wc -l  # Must be 0
    ```
    For personal names, check:
    ```bash
    git ls-files | xargs grep -l "Ossie\|Irondi" 2>/dev/null
    ```
  ...
  <done>
    Zero tracked files contain the Fidelity account number, Google Sheets ID, or employer names. Personal name references are either removed or replaced with template variables. Any remaining "Ossie" references are documented with justification (e.g., in git config, author attribution).
  </done>
</task>
...
git ls-files | xargs grep -rlE "Z05724592|1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4|admin@kamdental|admin@unifiedental" 2>/dev/null | wc -l

# Employer names in PII context (must be zero)
git ls-files | xargs grep -rl "\bAvanade\b" 2>/dev/null | wc -l
...
<success_criteria>
1. Zero tracked files contain Z05724592, the Google Sheets spreadsheet ID, or employer names
