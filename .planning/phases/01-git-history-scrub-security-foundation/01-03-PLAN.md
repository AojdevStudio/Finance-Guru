...
<task type="auto">
  ...
  <action>
    **File 1: `.gitleaks.toml`**
    ...
    3. **personal-llc-names** - Regex: `(?i)(MaryFinds|KC\s*Ventures\s*Consulting\s*Group)\s*LLC` - Catches known LLC business entity names
    4. **employer-names** - Regex: `\b(Avanade)\b` - Catches known employer names. NOTE: Exclude "CBN" from this rule due to false positive risk -- it's only 3 characters. The CI grep (Task 2) will handle CBN-specific patterns with more context.
    5. **personal-names** - Regex: `(?i)\b(Irondi)\b` - Catches personal family name. NOTE: Exclude "Ossie" as first name alone could cause false positives in other contexts. Use "Irondi" which is unique enough.
    ...
    **Test the hook works by attempting a PII commit:**
    ```bash
    echo "Z05724592" > /tmp/test-pii.txt
  ...
  <action>
    ...
    **Job 1: Working tree PII scan (custom grep)**
    ...
    - Grep all tracked files for specific known PII patterns:
      `Z05724592|Z0[0-9]{7,}|MaryFinds|KC.Ventures|1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4|admin@kamdental|admin@unifiedental|9424526a[a-f0-9]{56}`
    ...
    **File 2: `scripts/qa/pii-audit.sh`**
    ...
    SPECIFIC_PATTERNS="Z05724592|Z0[0-9]{7,}|MaryFinds|KC.Ventures|1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4|admin@kamdental|admin@unifiedental|9424526a[a-f0-9]{56}"
...
echo "Z05724592" > test-pii-temp.txt
git add test-pii-temp.txt
