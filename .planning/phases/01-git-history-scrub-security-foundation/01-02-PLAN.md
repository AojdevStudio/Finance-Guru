...
must_haves:
  truths:
    - "Running `git log --all -p | grep -ciE 'Z05724592|MaryFinds|KC Ventures|1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4|admin@kamdental|admin@unifiedental'` returns 0 on the filtered clone"
...
<task type="auto">
  ...
  <action>
    ...
    **File 1: `scripts/qa/pii-replacements.txt`**
    ...
    # === HIGH: LLC Names ===
    literal:MaryFinds LLC==>REDACTED_LLC
    literal:KC Ventures Consulting Group LLC==>REDACTED_LLC
    literal:KC Ventures==>REDACTED_LLC

    # === MEDIUM: Employer Names ===
    literal:Avanade==>REDACTED_EMPLOYER
    ...
    **IMPORTANT on CBN:** Do NOT add a blanket `literal:CBN==>REDACTED` replacement. "CBN" is only 3 characters and would cause collateral damage. Instead, use context-specific patterns:
    ```
    literal:CBN 401(k)==>REDACTED_EMPLOYER 401(k)
    literal:CBN 401k==>REDACTED_EMPLOYER 401k
    ```
    ...
    **File 2: `scripts/qa/author-mailmap.txt`**
    ...
    Finance Guru Developer <dev@example.com> Ossie Irondi <admin@kamdental.com>
    ```
  ...
  <verify>
    ...
    grep "Z05724592" scripts/qa/pii-replacements.txt  # Must find the account number
...
<task type="auto">
  ...
  <action>
    ...
    **Step 4: Verify scrub is complete -- SPECIFIC known PII (MUST be zero)**
    ```bash
    SPECIFIC_COUNT=$(git log --all -p | grep -ciE "Z05724592|MaryFinds|KC Ventures|1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4|admin@kamdental|admin@unifiedental|9424526a719032acbe090cc883accedb1b7eb167e89f855d78a7a0fec0aaf441" || true)
  ...
  <verify>
    ...
    ```bash
    # Zero specific PII
    git log --all -p | grep -ciE "Z05724592|MaryFinds|KC Ventures|1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4|admin@kamdental|admin@unifiedental|9424526a" | head -1
    # Must output: 0
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    2. Run the full PII audit:
       ```bash
       git log --all -p | grep -ciE "Z05724592|MaryFinds|KC Ventures|1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4|admin@kamdental|admin@unifiedental|9424526a"
       ```
       Must return 0.
    ...
    5. Spot-check the working tree: `grep -r "Z05724592" . 2>/dev/null` -- must return nothing.
...
git log --all -p | grep -ciE "Z05724592|MaryFinds|KC Ventures|1HtHRP3CbnOePb8RQ0RwzFYOQxk0uWC6L8ZMJeQYfWk4|admin@kamdental|admin@unifiedental|9424526a719032acbe090cc883accedb1b7eb167e89f855d78a7a0fec0aaf441"
