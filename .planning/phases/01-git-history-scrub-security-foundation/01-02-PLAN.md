...
must_haves:
  truths:
    - "Running `git log --all -p | grep -ciE '{account_id}|{llc_name}|{llc_name}|{spreadsheet_id}|{personal_email_1}|{personal_email_2}'` returns 0 on the filtered clone"
    - "Running `git log --all --format='%an <%ae>' | sort -u` shows no personal email addresses (kamdental, unifiedental)"
...
<task type="auto">
  ...
  <action>
    ...
    **File 1: `scripts/qa/pii-replacements.txt`**
    ...
    # === HIGH: LLC Names ===
    literal:{llc_name} LLC==>REDACTED_LLC
    literal:{llc_name}==>REDACTED_LLC
    literal:{llc_name}==>REDACTED_LLC

    # === MEDIUM: Employer Names ===
    literal:{employer_name}==>REDACTED_EMPLOYER

    # === MEDIUM: Personal Names ===
    literal:{user_name}==>REDACTED_NAME
    literal:{owner_first_name}==>REDACTED_NAME

    # === HIGH: Email Addresses (in content, not author -- author handled by mailmap) ===
    literal:{personal_email_1}==>redacted@example.com
    literal:{personal_email_2}==>redacted@example.com
    ...
    literal:192000==>REDACTED_AMOUNT
    ...
    **IMPORTANT on CBN:** Do NOT add a blanket `literal:CBN==>REDACTED` replacement. "CBN" is only 3 characters and would cause collateral damage. Instead, use context-specific patterns:
    ```
    literal:{employer_name} 401(k)==>REDACTED_EMPLOYER 401(k)
    literal:{employer_name} 401k==>REDACTED_EMPLOYER 401k
    ```
    ...
    **File 2: `scripts/qa/author-mailmap.txt`**
    ...
    ```
    Finance Guru Developer <dev@example.com> AOJDevStudio <{personal_email_2}>
    Finance Guru Developer <dev@example.com> AOJDevStudio <{personal_email_1}>
    Finance Guru Developer <dev@example.com> {user_name} <{personal_email_2}>
    Finance Guru Developer <dev@example.com> {user_name} <{personal_email_1}>
    ```
  ...
  <verify>
    ...
    grep "{account_id}" scripts/qa/pii-replacements.txt  # Must find the account number
    grep "9424526a" scripts/qa/pii-replacements.txt  # Must find the API token
    grep "kamdental" scripts/qa/author-mailmap.txt  # Must find the email
    ```
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    **Step 4: Verify scrub is complete -- SPECIFIC known PII (MUST be zero)**
    ```bash
    SPECIFIC_COUNT=$(git log --all -p | grep -ciE "{account_id}|{llc_name}|{llc_name}|{spreadsheet_id}|{personal_email_1}|{personal_email_2}|9424526a719032acbe090cc883accedb1b7eb167e89f855d78a7a0fec0aaf441" || true)
    ...
    **Step 5: Verify author metadata is clean**
    ...
    # Must NOT contain kamdental or unifiedental emails
    ```
  ...
  <verify>
    ...
    ```bash
    # Zero specific PII
    git log --all -p | grep -ciE "{account_id}|{llc_name}|{llc_name}|{spreadsheet_id}|{personal_email_1}|{personal_email_2}|9424526a" | head -1
    # Must output: 0

    # No personal emails in author metadata
    git log --all --format='%ae' | grep -iE "kamdental|unifiedental" | wc -l
    # Must output: 0
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    2. Run the full PII audit:
       ```bash
       git log --all -p | grep -ciE "{account_id}|{llc_name}|{llc_name}|{spreadsheet_id}|{personal_email_1}|{personal_email_2}|9424526a"
       ```
       Must return 0.
    3. Check author identities: `git log --all --format='%an <%ae>' | sort -u`
       Must NOT show kamdental or unifiedental emails.
    4. Spot-check a few recent commits: `git log --oneline -10` -- look for any PII in messages.
    5. Spot-check the working tree: `grep -r "{account_id}" . 2>/dev/null` -- must return nothing.
...
git log --all -p | grep -ciE "{account_id}|{llc_name}|{llc_name}|{spreadsheet_id}|{personal_email_1}|{personal_email_2}|9424526a719032acbe090cc883accedb1b7eb167e89f855d78a7a0fec0aaf441"

# Tier 2: Author metadata (MUST be 0)
git log --all --format='%ae' | grep -ciE "kamdental|unifiedental"
