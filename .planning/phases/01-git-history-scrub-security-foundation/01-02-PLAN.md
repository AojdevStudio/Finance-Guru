...
must_haves:
  truths:
    - "Running `git log --all -p | grep -ciE '{account_id}|{llc_name}|{llc_name}|{spreadsheet_id}|admin@kamdental|admin@unifiedental'` returns 0 on the filtered clone"
...
<task type="auto">
  ...
  <action>
    ...
    **File 2: `scripts/qa/author-mailmap.txt`**
    ...
    Finance Guru Developer <dev@example.com> {user_name} <admin@unifiedental.com>
    Finance Guru Developer <dev@example.com> {user_name} <admin@kamdental.com>
    ```
...
<task type="auto">
  ...
  <action>
    ...
    **Step 4: Verify scrub is complete -- SPECIFIC known PII (MUST be zero)**
    ```bash
    SPECIFIC_COUNT=$(git log --all -p | grep -ciE "{account_id}|{llc_name}|{llc_name}|{spreadsheet_id}|admin@kamdental|admin@unifiedental|9424526a719032acbe090cc883accedb1b7eb167e89f855d78a7a0fec0aaf441" || true)
  ...
  <verify>
    ...
    ```bash
    # Zero specific PII
    git log --all -p | grep -ciE "{account_id}|{llc_name}|{llc_name}|{spreadsheet_id}|admin@kamdental|admin@unifiedental|9424526a" | head -1
    # Must output: 0
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    2. Run the full PII audit:
       ```bash
       git log --all -p | grep -ciE "{account_id}|{llc_name}|{llc_name}|{spreadsheet_id}|admin@kamdental|admin@unifiedental|9424526a"
       ```
       Must return 0.
...
git log --all -p | grep -ciE "{account_id}|{llc_name}|{llc_name}|{spreadsheet_id}|admin@kamdental|admin@unifiedental|9424526a719032acbe090cc883accedb1b7eb167e89f855d78a7a0fec0aaf441"
