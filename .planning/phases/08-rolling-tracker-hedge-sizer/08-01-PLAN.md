...
must_haves:
  truths:
    ...
    - "import_from_csv() parses a Fidelity CSV export and imports options positions into positions.yaml"
...
<objective>
Create the RollingTracker calculator class (Layer 2) that manages hedge positions: loading from YAML, calculating status with live prices, suggesting rolls, logging new positions and rolls, importing from Fidelity CSV, and reading history.

Purpose: This is the core business logic for the rolling tracker CLI. It handles all position lifecycle operations: entry (manual and CSV import), monitoring, roll suggestion, roll execution recording, and history retrieval. It wraps the existing scan_chain() function to suppress stderr noise and applies an American-style intrinsic value floor to Black-Scholes put pricing.
...
def get_status(self) -> list[dict]:
    ...
    - urgency: str ("ROLL" if 3 < dte <= 7, "EXPIRING" if 0 < dte <= 3, "EXPIRED" if dte <= 0, "" otherwise)
    """
```
Implementation:
...
```
  ...
  <done>RollingTracker class exists with __init__, _scan_chain_quiet, _price_put_american, load_positions, _save_positions, and get_status methods. American put pricing applies intrinsic value floor (BS-01). scan_chain stderr is suppressed. Urgency markers: EXPIRED (DTE<=0), EXPIRING (0<DTE<=3), ROLL (3<DTE<=7), none (DTE>7).</done>
</task>

<task type="auto">
  <name>Task 2: Add roll suggestion, position logging, CSV import, and history methods</name>
  <files>src/analysis/rolling_tracker.py</files>
  <action>
...
**`import_from_csv()` method:**
```python
def import_from_csv(self, csv_path: str | Path) -> list[dict]:
    """
    Import options positions from a Fidelity CSV export.

    Parses the CSV file looking for options positions and adds them
    to positions.yaml. Skips non-options rows (stocks, mutual funds).

    Expected CSV columns (Fidelity portfolio export format):
    - Symbol (contains OCC-style option symbols like '-QQQ260417P420')
    - Description (contains 'PUT' or 'CALL')
    - Quantity
    - Last Price or Current Value
    - Cost Basis Total or Cost Basis Per Share

    The method parses option symbols to extract:
    - ticker: underlying symbol (e.g., 'QQQ')
    - strike: strike price
    - expiry: expiration date
    - hedge_type: 'put' or 'call' based on Description or symbol

    Args:
        csv_path: Path to Fidelity CSV export file

    Returns:
        List of position dicts that were imported.
...
```
Implementation:
1. Open and read the CSV file with `csv.DictReader`
2. Iterate rows, identify options positions by checking:
   - Symbol column contains option indicator (typically starts with '-' for Fidelity, or contains date/strike pattern)
   - Description column contains 'PUT' or 'CALL'
3. For each options row, parse the OCC-style symbol to extract:
   - ticker (underlying, e.g., 'QQQ')
   - expiry date (from the symbol's date portion, format YYMMDD)
   - strike price (from the symbol's strike portion)
   - type: 'put' or 'call' from 'P' or 'C' in symbol
4. Build position_data dict:
   ```python
   {
       "ticker": ticker,
       "hedge_type": "put" or "call",
       "strike": strike,
       "expiry": expiry_date.isoformat(),
       "quantity": abs(int(quantity)),  # Fidelity may show negative for short positions
       "premium_paid": cost_basis_per_share or last_price,
       "entry_date": date.today().isoformat(),
       "contract_symbol": raw_symbol,
   }
   ```
5. Validate each position via `HedgePosition.model_validate()` before adding
6. Call `log_position()` for each valid position
7. Return list of all imported positions
8. Handle edge cases:
   - Skip rows where parsing fails (log a warning, continue to next row)
   - Skip non-options rows (stocks, cash, mutual funds)
   - Handle both positive and negative quantities (Fidelity shows negative for short positions)
...
- CSV with no options rows (import_from_csv returns empty list)
- CSV with malformed option symbols (skip row, continue)
  </action>
  <verify>
```bash
uv run python -c "
...
# Verify import_from_csv method exists
assert hasattr(tracker, 'import_from_csv'), 'Missing import_from_csv method'
print('import_from_csv method present')

print('All methods OK')
"
```
  </verify>
  <done>RollingTracker has suggest_roll (RT-02, RT-03), log_position, import_from_csv (Fidelity CSV parsing), log_roll, get_history methods. Roll suggestions ranked by per-day cost with top 3 candidates. Position and roll data persisted to YAML files. CSV import parses OCC option symbols for ticker, strike, expiry, type.</done>
</task>
...
uv run python -c "
from src.analysis.rolling_tracker import RollingTracker
methods = ['load_positions', 'get_status', 'suggest_roll', 'log_position', 'import_from_csv', 'log_roll', 'get_history']
for m in methods:
...
4. get_status() enriches positions with DTE, current_value, P&L, urgency markers (EXPIRED <=0, EXPIRING 0<DTE<=3, ROLL 3<DTE<=7, none >7)
...
7. import_from_csv() parses Fidelity CSV exports, extracts options positions, imports into positions.yaml
8. log_roll() moves position, appends to roll-history.yaml
9. get_history() reads from roll-history.yaml
10. All methods handle empty/missing YAML files gracefully
</success_criteria>
