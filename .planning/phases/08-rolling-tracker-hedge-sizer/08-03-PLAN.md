...
must_haves:
  truths:
    ...
    - "DTE urgency shown with text markers [ROLL] and [EXPIRING] alongside color when available"
  artifacts:
    - path: "src/analysis/rolling_tracker_cli.py"
      provides: "CLI with 5 argparse subcommands: status, suggest-roll, log-position, log-roll, history"
      exports: ["main"]
  key_links:
...
<objective>
Create the rolling tracker CLI (Layer 3) with argparse subcommands for position management: status, suggest-roll, log-position, log-roll, and history.

Purpose: This is the first argparse subcommand CLI in the codebase (HEDG-04), establishing the pattern for future tools that need multiple operations. It provides the user-facing interface for all position lifecycle operations, with both human-readable ASCII tables and JSON output.

Output: `src/analysis/rolling_tracker_cli.py` with 5 subcommands and --output json support.
</objective>
...
#!/usr/bin/env python3
"""
Rolling Tracker CLI for Finance Guru™
...
5. DTE urgency rendering:
   - DTE > 14: plain text (green if terminal supports it)
   - DTE 7-14: "[ROLL]" text marker (yellow if supported)
   - DTE <= 7: "[EXPIRING]" text marker (red if supported)
   - DTE <= 0: "[EXPIRED]" text marker
...
```
  ...
  <done>rolling_tracker_cli.py exists with argparse subcommands skeleton, status shows ASCII table with DTE markers, suggest-roll shows replacement candidates, both support --output json</done>
</task>

<task type="auto">
  <name>Task 2: Add log-position, log-roll, and history subcommands</name>
  <files>src/analysis/rolling_tracker_cli.py</files>
  <action>
Add three more subcommands to the rolling tracker CLI.
...
```
  ...
  <done>rolling_tracker_cli.py has all 5 subcommands (status, suggest-roll, log-position, log-roll, history) with --output json support, educational disclaimer, and proper error handling. HEDG-04 complete.</done>
</task>
...
```bash
# 1. All 5 subcommands have --help
for cmd in status suggest-roll log-position log-roll history; do
...
uv run python src/analysis/rolling_tracker_cli.py --help | grep -E "status|suggest-roll|log-position|log-roll|history"
...
2. 5 subcommands work: status, suggest-roll, log-position, log-roll, history
3. status displays ASCII table with DTE urgency markers [ROLL], [EXPIRING] (RT-01, RT-02)
...
6. log-roll closes old position and opens new one
7. history shows all historical rolls
8. All subcommands support --output json (STD-01)
9. Educational disclaimer on all output (STD-02, XC-05)
10. --help shows complete examples (STD-03)
</success_criteria>
