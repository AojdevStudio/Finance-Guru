...
Purpose: This completes the user-facing layer of Phase 9 (Layer 3 CLI), validates the calculator with known-answer tests (STD-04, HEDG-13, XC-04), creates the M2 architecture diagram (HEDG-11), and verifies the hedge_comparison_cli.py integration (part of HEDG-12). The CLI is the entry point for users and agents -- it must support --scenarios, --output json, and --help with complete examples.
...
@.planning/ROADMAP.md
@.planning/STATE.md
@.planning/phases/09-sqqq-vs-puts-comparison/09-RESEARCH.md
@.planning/phases/09-sqqq-vs-puts-comparison/09-01-SUMMARY.md
...
Note: Phases 6, 7, and 8 components may not exist yet in the codebase. The diagram shows the PLANNED M2 architecture. Label non-existent components with "(Phase N)" annotations.
...
The full HEDG-12 requirement (all 4 CLIs) depends on Phases 7 and 8. For Phase 9, only verify hedge_comparison_cli works. Add a comment in the test file noting that full HEDG-12 integration requires all 4 phases complete.
  </action>
  <verify>
...
<output>
After completion, create `.planning/phases/09-sqqq-vs-puts-comparison/09-02-SUMMARY.md`
</output>
