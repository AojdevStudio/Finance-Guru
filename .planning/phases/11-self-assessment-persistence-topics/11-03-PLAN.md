...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/explorer/dist/options-greeks-explorer.html"
      provides: "New options Greeks explorer with 30 concept nodes"
      contains: "options-greeks"
    - path: "src/explorer/dist/risk-management-explorer.html"
      provides: "New risk management explorer with 27 concept nodes"
...
<task type="auto">
  ...
  <action>
    ...
    **Step 3: Verify EXPL-16 performance (sub-1-second load)**
    ...
    Note: Explorers have 21-30 nodes (dividend-strategy: 21, risk-management: 27, options-greeks: 30). With Cytoscape.js CoSE layout using `animate: false` and `numIter: 2000`, layout computation is <100ms on any modern device even at 30 nodes. The sub-1-second load target is met by design given zero external dependencies and moderate file sizes.
    ...
    **Step 4: Cross-plan wiring verification**
    ...
    - `options-greeks: WIRING OK - 30 nodes, options-greeks`
    - `risk-management: WIRING OK - 27 nodes, risk-management`
  ...
  <how-to-verify>
    ...
    **Test each explorer (start with options-greeks as the primary test):**
    ...
    5. **SC-4: Options-greeks explorer**
       - Verify ~30 concept nodes are visible with 7 category colors (including second-order Greeks and volatility surface).
       ...
       - Click "Second-Order Greeks" -- should show charm, vanna, volga/vomma, color.
       - Click "Volatility Surface" -- should show volatility surface and volatility effects nodes.
       - Click "All" to restore all nodes.

    6. **SC-4: Risk-management explorer**
       - Verify ~27 concept nodes are visible with 6 category colors (including advanced risk analytics).
       - Hover over "VaR" or "Sharpe Ratio" -- descriptions should be educational and detailed.
       - Preset filters: click "Risk Measurement" -- should show risk measurement nodes (including drawdown-duration).
       - Click "Advanced Analytics" -- should show stress-testing, factor-exposure, regime-detection, conditional-correlation.
...
4. Options-greeks explorer: 30 nodes, 37 edges, 7 categories, 6 presets
5. Risk-management explorer: 27 nodes, 35 edges, 6 categories, 6 presets
