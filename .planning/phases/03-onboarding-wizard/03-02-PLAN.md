...
2. **`SECTION_ORDER` list:** Ordered list of (SectionName, runner_function) tuples defining the 8-section flow:
   ```
   liquid_assets -> investments -> cash_flow -> debt -> preferences -> summary -> broker -> env_setup
   ```
...
5. **`run_wizard() -> None`:**
   - Print large ASCII art welcome banner for Finance Guru branding (design is Claude's discretion) + brief explanation of what onboarding does + total section count (8 sections) + list of files that will be created (user-profile.yaml, CLAUDE.md, .env, mcp.json)
   - Create fresh OnboardingState
   - Build a `section_runners` dict mapping financial section names to their runner functions (for the summary section's re-edit loop): `{"liquid_assets": run_liquid_assets_section, "investments": run_investments_section, "cash_flow": run_cash_flow_section, "debt": run_debt_section, "preferences": run_preferences_section}`
   - Create a helper function `print_progress(current: int, total: int)` that prints a visual progress bar before each section (e.g., `[████████░░░░░░░░] 50%` -- exact character style and width are Claude's discretion). This is displayed IN ADDITION TO the "Section N of 8: Name" headers that each section runner already prints.
   - Iterate through SECTION_ORDER, calling `print_progress(i, total)` then each runner with state. For the summary section, also pass the `section_runners` dict as the second argument.
   - Two-stage flow: Stage 1 (financial sections 1-5 + summary review at position 6), then Stage 2 (config sections: broker, env_setup at positions 7-8). After ALL 8 sections complete, call convert_state_to_user_data.
   - Call generate_config_files
   - Print celebratory ASCII art completion banner (design is Claude's discretion) + list each generated file with a checkmark beside it (e.g., "  ✓ fin-guru-private/fin-guru/data/user-profile.yaml") + "Next steps" section telling user to launch Claude Code and activate Finance Guru
...
All three commands exit 0.
  ...
  <done>
Wizard CLI exists at src/cli/onboarding_wizard.py with: two-stage 8-section orchestration (Stage 1 financial + summary review at position 6, Stage 2 config at positions 7-8), section_runners dict passed to summary section for targeted re-edit loop, state-to-UserDataInput conversion with explicit string-to-enum mapping (RiskTolerance("aggressive") etc.), config file generation to correct paths (private files to fin-guru-private/, CLAUDE.md and .env to project root, mcp.json to .claude/ via explicit Path.write_text), backup of existing files before overwriting, and argparse entry point with --dry-run flag.
  </done>
</task>
...
5. `test_summary_section_confirmed(mocker)`:
   - Pre-populate state.data with mock data for all 5 financial sections
   - Create a section_runners dict with mock/real runner functions
   - Mock questionary.confirm().ask() to return True (user confirms on first try)
   - Call run_summary_section(state, section_runners)
   - Assert state.data["summary"]["confirmed"] == True

6. `test_summary_section_reedit_loop(mocker)`:
   - Pre-populate state.data with mock data for all 5 financial sections
   - Create a section_runners dict with mock runner functions for each financial section
   - Mock questionary.confirm().ask() side_effect: [False, True] (reject first, confirm second)
   - Mock questionary.select().ask() to return "1. Liquid Assets" (or the matching choice string)
   - The mock runner for liquid_assets should update state.data with new values
   - Call run_summary_section(state, section_runners)
   - Assert the liquid_assets mock runner was called once (re-edit)
   - Assert state.data["summary"]["confirmed"] == True (confirmed on second pass)
...
4. `test_section_order_has_eight_entries()`:
   ...
   - Assert first is liquid_assets, last is env_setup
   - Assert summary is at position 6 (index 5), verifying two-stage flow: Stage 1 financial (1-5) + summary (6), Stage 2 config (7-8)
...
2. Wizard orchestrates all 8 sections in two-stage flow: Stage 1 financial (liquid_assets -> investments -> cash_flow -> debt -> preferences -> summary/review), then Stage 2 config (broker -> env_setup)
