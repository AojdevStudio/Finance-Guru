...
must_haves:
  truths:
    ...
    - "Collected financial data is complete and ready for configuration file generation"
  artifacts:
...
2. `run_investments_section(state) -> OnboardingState`:
   ...
   - For select prompts: present human-readable labels (e.g., "Aggressive Growth", "Moderate") as choices, store the raw human-readable string that questionary returns (e.g., "aggressive", "moderate"). The string-to-enum conversion happens later in Plan 02's convert_state_to_user_data.
...
5. `run_preferences_section(state) -> OnboardingState`:
   ...
   - For select prompts: present human-readable labels, store the raw string questionary returns. String-to-enum conversion happens in Plan 02.
   - Store as dict matching UserPreferencesInput fields
   - Next: summary

6. `run_summary_section(state, section_runners: dict[str, Callable]) -> OnboardingState`:
   - Does NOT prompt for new data
   - This is the Stage 1 review gate -- user reviews all financial data before moving to config sections
   - Prints formatted summary of completed Stage 1 financial sections (liquid_assets, investments, cash_flow, debt, preferences) from state.data (currency formatted, percentages displayed nicely)
   - Asks questionary.confirm: "Does this look correct?"
   - If "yes": mark summary complete, proceed (wizard CLI continues to Stage 2 config sections: broker, env_setup)
   - If "no": implement targeted re-edit loop:
     a. Show numbered list of Stage 1 financial sections via questionary.select with choices: "1. Liquid Assets", "2. Investments", "3. Cash Flow", "4. Debt", "5. Preferences"
     b. User picks which section to redo
     c. Look up the chosen section's runner function from section_runners dict and call it with current state
     d. Update state with the re-run section's returned state
     e. Loop back to: re-print summary, ask "Does this look correct?" again
     f. Continue looping until user confirms
   - The section_runners parameter is a dict mapping section name strings to runner functions, e.g.: `{"liquid_assets": run_liquid_assets_section, "investments": run_investments_section, "cash_flow": run_cash_flow_section, "debt": run_debt_section, "preferences": run_preferences_section}`
   - Handle None returns from questionary (Ctrl+C) -- break out of loop and return state as-is without marking complete
   - Store: `{"confirmed": bool}`
   - Next: broker (Stage 2 begins)

7. `run_broker_section(state) -> OnboardingState`:
...
8. `run_env_setup_section(state) -> OnboardingState`:
...
- Import enums (RiskTolerance, AllocationStrategy, InvestmentPhilosophy) from yaml_generation_inputs.py for building select choice lists. Use `[e.value for e in EnumClass]` to generate the choice labels.
- **Storage format:** Section runners store the human-readable strings that questionary returns in state.data (e.g., `"aggressive"`, `"growth"`). They do NOT convert to enum instances. The conversion from string to enum instance happens in Plan 02's `convert_state_to_user_data` function, which maps strings like `"aggressive"` to `RiskTolerance.AGGRESSIVE` when constructing the UserDataInput Pydantic model.
...
Exits 0 (no output means success).
  ...
  <done>
All 8 section runner functions exist in onboarding_sections.py. Each uses ask_with_retry for validated prompts, questionary for interactive input, and stores raw data (human-readable strings for enums, floats for currencies, decimals for rates) in OnboardingState.data. Question flow mirrors the TypeScript scaffold. Section ordering follows the two-stage flow: Stage 1 financial sections (liquid_assets -> investments -> cash_flow -> debt -> preferences) end with summary/review at position 6, then Stage 2 config sections (broker -> env_setup) follow. The summary section accepts a section_runners dict and implements a targeted re-edit loop: if the user says data is wrong, they pick a specific financial section from a numbered list via questionary.select, that section re-runs, then returns to summary for re-confirmation. Enum values from yaml_generation_inputs.py are used to build select choice lists, but the stored values are the raw strings returned by questionary -- enum instance conversion is deferred to Plan 02's convert_state_to_user_data.
  </done>
</task>
...
8. Section runners store raw strings for enum fields (NOT enum instances) -- conversion deferred to Plan 02
9. No modifications to existing yaml_generation_inputs.py or yaml_generator.py
10. Existing pytest suite passes without regressions
</success_criteria>
