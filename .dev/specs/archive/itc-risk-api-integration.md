---
title: "Into The Cryptoverse (ITC) Risk Models API Integration"
status: implemented
created: 2026-01-09
updated: 2026-02-03
author: "{user_name}"
...
tags:
  ...
  - market-data
references:
  - agent-itc-integration-compliance.md
supersedes: []
diagrams:
  human: ""
  machine: ""
...
- **Tests:** `uv run pytest tests/python/test_itc_risk.py -v -k "TestITCRiskResponse"`
...
- **Tests:** `uv run pytest tests/python/test_itc_risk.py -v -k "TestITCRiskCalculator"`
...
- **Files:** `tests/python/test_itc_risk.py`
- **Acceptance:** All unit tests pass, covers models, calculator, and CLI
- **Tests:** `uv run pytest tests/python/test_itc_risk.py -v`
...
- **Tests:** `grep -l "<itc-risk-integration>" .claude/commands/fin-guru/agents/*.md | wc -l` (should be 4 including Compliance Officer)
...
## Implementation Close-Out (2026-02-03)
...
- ✅ **Docs updated**:
  - `CLAUDE.md` includes `itc_risk_cli.py` in tool list + agent-tool matrix
- ✅ **Agent prompts updated**:
  - Market Researcher / Strategy Advisor / Quant Analyst include `<itc-risk-integration>`
  - Compliance Officer integration is tracked via `agent-itc-integration-compliance.md`
- ✅ **Tests executed**:
  - `uv run pytest tests/python/test_itc_risk.py -v`
  - Result: **31 passed, 2 skipped** (integration tests skipped without `ITC_API_KEY`)

### Highlighted Deltas / Cleanup Items (non-blocking)

- ⚠️ **FR5 default nearest-band count**: spec says “nearest 5 by default”; implementation currently uses 7 (`ITCRiskResponse.get_nearest_bands(7)` in `src/analysis/itc_risk_cli.py`).
- ⚠️ **`.env.example` comment**: `ITC_API_KEY` is present, but the requested “Into The Cryptoverse Risk Models API” comment header is not currently present.
- ⚠️ **`.env` loading behavior**: `src/analysis/itc_risk_cli.py` / `src/analysis/itc_risk.py` do not call `load_dotenv()`; `ITC_API_KEY` must be exported in the environment (or dotenv loading added) for the CLI to work when only `.env` is populated.
- ⚠️ **Executable bit**: spec suggested `chmod +x src/analysis/itc_risk_cli.py`; file has a shebang but is not currently executable.
...
**Status:** ✅ Implemented (verified 2026-02-03)
