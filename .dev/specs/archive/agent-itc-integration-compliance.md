...
- ✅ **Agent prompt updated**: `.claude/commands/fin-guru/agents/compliance-officer.md` includes:
  - `ITC` callout in `<critical-actions>`
  - `<itc-risk-integration>` block with supported tickers + workflows
  - divergence + decision guidance
- ✅ **No code changes required**: relies on existing `src/analysis/itc_risk_cli.py`
