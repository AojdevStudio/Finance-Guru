...
class PortfolioPriceData(BaseModel):
    ...
    tickers: list[str] = Field(
        ..., min_length=2, description="Asset ticker symbols (min 2 for correlation)"
    )
    dates: list[date] = Field(
        ..., min_length=30, description="Trading dates (must be same for all assets)"
    )
...
class RollingCorrelationOutput(BaseModel):
    ...
    current_correlation: float = Field(
        ..., ge=-1.0, le=1.0, description="Most recent correlation value"
    )
    average_correlation: float = Field(
        ..., ge=-1.0, le=1.0, description="Average correlation over the period"
    )
