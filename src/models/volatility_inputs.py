...
class VolatilityDataInput(BaseModel):
    ...
    dates: list[date] = Field(
        ..., min_length=20, description="Trading dates (min 20 days)"
    )
...
class VolatilityConfig(BaseModel):
    ...
    atr_period: int = Field(
        default=14, ge=7, le=30, description="ATR calculation period (default: 14)"
    )
...
class HistoricalVolatilityOutput(BaseModel):
    ...
    daily_volatility: float = Field(
        ..., ge=0.0, description="Daily volatility (std dev)"
    )
