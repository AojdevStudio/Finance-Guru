...
Base: `uv run python <script> <ticker(s)> [flags]`;Common Flags: `--days N` (90=quarter,252=year),`--output json`,`--benchmark SPY`;Example Tickers: TSLA,PLTR,NVDA,SPY;Portfolio Loop: `for ticker in TSLA PLTR NVDA; do uv run python <tool> $ticker [flags]; done`;Package Management: `uv sync` (install),`uv add/remove <pkg>` (manage);Market Data: `uv run python -m src.utils.market_data TSLA [PLTR AAPL ...]`;Tests: `uv run pytest` (unit),`uv run pytest -m "not integration"` (skip API tests);Lint: `uv run ruff format .` and `uv run ruff check .` (format+lint),`uv run mypy src/` (type check);Justfile: `just --list` (see all recipes),`just load-diagrams` (load mermaid context)
